# Swiflo

> **Low-fee Gulf → Nepal remittance on Solana with instant NPR payout**

Swiflo is a blockchain-powered remittance platform built for the Gulf-to-Nepal corridor. We are a small team building infrastructure that lets migrant workers send money home faster, cheaper, and more transparently than any traditional service.

---

## Who We Are

Swiflo is a fintech protocol on Solana that connects Gulf-based migrant workers directly to their families in Nepal — cutting out the middlemen that eat into every transfer. We replace the opaque fee structures of legacy remittance providers with a simple, auditable, on-chain system.

## The Problem

~500,000 Nepali workers live and work in the Gulf (UAE, Qatar, Saudi Arabia, Kuwait). They send billions of rupees home every year to support their families. But every transfer is taxed by intermediaries — Western Union offers a rate of **148.2560 NPR/USD** when the real mid-market rate is **150.99 NPR/USD**, silently taking 1.8% off the top. Hawala networks charge 3–5% with zero transparency. Transfers take 1–3 days, and recipients have no way to track their money.

**On every $500 sent, families lose Rs 1,065 to Western Union alone.**

## Our Solution

Swiflo uses USDC on Solana as the transfer rail. A Gulf worker deposits USDC, signs a transaction with their Privy-powered embedded wallet, and the funds are locked in on-chain escrow. The MTO (Money Transfer Operator) in Nepal instantly pays the recipient's eSewa wallet from its own NPR reserve. Later, settlement rails (Bridge/Thunes class) replenish NPR reserve and escrow is released back to Swiflo's liquidity vault.

**Implemented in this repo today:** on-chain escrow (`initiateTransfer`), vault advance to MTO (`advanceToMto`), MTO disbursement confirmation (`confirmDisbursement`), and settler lifecycle (`settleTransfer` + `replenishVault` + `collectFees`).


## What Our Product Does

A sender opens the Swiflo web app, logs in with their email or Google account (no crypto experience needed — Privy creates a Solana wallet for them in the background), enters the amount in USDC and the recipient's eSewa phone number, and clicks send. The app fetches a live NPR rate from Pyth oracle, shows the recipient how much NPR they will receive versus what Western Union would give, and lets them confirm and sign the transaction. Within seconds, the MTO releases NPR to the recipient from its pre-funded reserve. Reserve replenishment is handled asynchronously as an internal settlement step in the demo, so the family gets paid instantly without waiting for cross-border settlement.

---

## Revenue Model

Swiflo earns on each transfer through a fee stack:

**Production target fee stack (commercial model): 1.0% total**

| Fee component | How it works |
|---|---|
| Bridge rail fee (0.3%) | External settlement rail cost for conversion/liquidity movement |
| Thunes payout fee (0.4%) | External payout network cost to deliver NPR via local rails |
| Swiflo platform fee (0.3%) | Swiflo protocol fee on transfer volume |
| LP yield allocation (0.1% of total transfer amount, from Swiflo fee) | Routed to liquidity providers who keep the vault funded |

**Current demo implementation:** on-chain logic currently uses `feeBps = 40` (0.40%) and does not execute live Bridge/Thunes API charges.

**Why senders still win:** Western Union's hidden exchange rate markup costs ~1.8%. Even at a 1.0% total target stack, Swiflo remains materially cheaper while preserving instant NPR payout UX.

**Revenue at scale:**
| Monthly volume | Swiflo revenue (0.3%) |
|---|---|
| $1M | $3,000 |
| $10M | $30,000 |
| $100M | $300,000 |

At $100M/month — a fraction of the $8B+ Gulf-to-Nepal annual remittance market — Swiflo generates $3.6M/year in protocol revenue with near-zero marginal cost per transaction (Solana fees are <$0.001 per tx).

**Secondary revenue streams (roadmap):**
- FX spread on currency conversion (USDC → NPR leg, once regulated)
- Premium B2B API for businesses sending bulk payroll to Nepal
- White-label protocol licensing to other corridors (Gulf → Bangladesh, Gulf → India)

---

## The Numbers

**On a $500 transfer** (mid-market rate: 1 USD = Rs 150.99):
| Service | How they charge | Recipient gets |
|---|---|---|
| Western Union | Rate markup (FX: 148.2560 NPR/USD, $0 fee) | Rs 74,128 |
| Hawala | 3–5% opaque markup | ~Rs 72,000–73,000 |
| **Swiflo** | **0.3% platform fee (+ partner rail fees when enabled), live Pyth rate** | **Varies by rail; still targeted below legacy effective cost** |

**At the platform-fee layer alone (0.3%), Swiflo remains cheaper than Western Union's typical effective FX markup. Final recipient amount depends on which external settlement rails are enabled.**

---

## Solution Architecture

<img src="./docs/images/architecture-diagram.png" alt="Swiflo architecture diagram" />

```
Gulf Worker (sender)                          Nepal (recipient)
┌─────────────────────┐                      ┌──────────────────────┐
│  Swiflo Web         │                      │  eSewa Wallet        │
│  (Privy Login)      │                      │  +977 98XXXXXXXX     │
└────────┬────────────┘                      └──────────┬───────────┘
         │ 1. Login with email/Google                   │
         │ 2. Send USDC → on-chain escrow               │
         ▼                                              │
┌─────────────────────┐                      ┌──────────┴──────────┐
│  Solana Programs    │                      │  MTO                │
│  remittance_pool    │◄────────────────────►│  (eSewa MTO)        │
│  liquidity_vault    │  advanceToMto +      │  pays NPR instantly │
└────────┬────────────┘  confirmDisbursement │  from NPR reserve   │
         │                                   └─────────────────────┘
         ▼                                              │
┌─────────────────────┐                                │
│  Swiflo Backend API │  later: settlement rails top-up │
│  (Fastify + Prisma) │◄───────────────────────────────┘
│  - Rate service     │     USDC → NPR for MTO reserve
│  - Settler (cron)   │
│  - Helius webhooks  │
└────────┬────────────┘
         │
         ▼
┌─────────────────────┐
│  Supabase (Postgres)│
│  transfers table    │
│  rateSnapshot table │
│  lpPosition table   │
└─────────────────────┘
```

---

## Complete Transfer Flow

```
Sender               API                 Solana Chain            MTO (eSewa)
  │                   │                       │                      │
  │─ POST /estimate ──►│                       │                      │
  │  { amountUsdc }   │─ Pyth Oracle ─────────►│                      │
  │                   │  USDC/USD + USD/NPR    │                      │
  │◄─ lockedRate ──────│                       │                      │
  │   recipientNpr    │                       │                      │
  │   savingsVsWU     │                       │                      │
  │                   │                       │                      │
  │─ POST /initiate ──►│                       │                      │
  │  (signs tx)       │── initiateTransfer ───►│                      │
  │                   │   USDC locked in       │                      │
  │                   │   on-chain escrow      │                      │
  │                   │                       │                      │
  │  (Helius webhook) ─►│                      │                      │
  │                   │── Create DB record     │                      │
  │                   │   status: PENDING      │                      │
  │                   │                       │                      │
│                   │── advanceToMto ────────►│                     │
│                   │   vault advances liquidity token value         │
  │                   │── POST /api/mto/disburse ─────────────────────►│
  │                   │   { phone, amountNpr } │                      │
  │                   │                       │            eSewa credit
  │                   │                       │◄── confirmDisbursement │
  │                   │                       │    MTO signs on-chain  │
  │                   │◄── webhook: DISBURSED ─────────────────────────│
  │                   │── status: DISBURSED    │                      │
  │                   │                       │                      │
  │              (after 30s, settler cron)     │                      │
  │                   │── settleTransfer ──────►│                     │
  │                   │   escrow → vault       │                      │
  │                   │── replenishVault ──────►│                     │
  │                   │   drops active_advances │                     │
  │                   │── collectFees ─────────►│                     │
  │                   │   fee → treasury       │                      │
  │                   │── status: SETTLED      │                      │
  │◄── Transfer complete ─│                    │                      │
```

**Key design:** The MTO pays the recipient from its own NPR reserve the moment eSewa confirms. Async reserve replenishment is modeled as internal settlement rails behavior, while escrow release to liquidity vault is handled by the on-chain settler lifecycle.

**On-chain instruction sequence:**
```
initiateTransfer → advanceToMto → confirmDisbursement → settleTransfer → replenishVault → collectFees
```

---

## Rate Architecture

Swiflo uses a 4-layer fallback to always show live NPR rates:

```
┌─────────────────────────────────────────────────────────┐
│                   Rate Service                          │
│                                                         │
│  Layer 1 (primary):                                     │
│  Pyth Hermes (USDC/USD) × ExchangeRate-API (USD/NPR)   │
│  ─────────────────────────────────────────────────────  │
│  USDC/USD feed: 0xeaa020c61cc479712813461ce153894a96a  │
│  USD/NPR from: api.exchangerate-api.com/v4/latest/USD  │
│                                                         │
│  Layer 2 (if ExchangeRate-API fails):                   │
│  ExchangeRate-API direct → USD/NPR only                 │
│                                                         │
│  Layer 3 (if Pyth + ExchangeRate both fail):            │
│  CoinGecko → tether USD/NPR                             │
│                                                         │
│  Layer 4 (last resort):                                 │
│  Last valid cached value, or 150.99 hardcoded           │
└─────────────────────────────────────────────────────────┘
                              │
                              ▼
              { nprPerUsd, source, cachedAt }
              30-second in-memory cache
```

**What the rate endpoint returns:**
```json
GET /api/rates?amount=100
{
  "nprPerUsd": 150.99,
  "source": "pyth+exchangerate-api",
  "cachedAt": "2026-04-28T10:32:00.000Z",
  "swifloNpr": 15039,
  "wuNpr": 14826,
  "savingsNpr": 213,
  "lockedRate": 150.99
}
```

---

## Liquidity Provider (LP) Flow

LPs deposit USDC into the `liquidity_vault` program. The vault advances liquidity token value to the MTO on each transfer (tracked as active advances), earns yield from the fee spread, and is replenished when transfers settle.

```
LP depositor
     │
     │── Deposit USDC to liquidity_vault
     │
     ▼
┌─────────────────────────────────────────┐
│  liquidity_vault (Solana program)       │
│                                         │
│  Total liquidity: 50,000 USDC           │
│  Active advances: 5,000 USDC (deployed) │
│  Available: 45,000 USDC                 │
│                                         │
│  Utilization = activeAdvances / total   │
│  APR scales 8% → 20% with utilization  │
│                                         │
│  Rewards claimable at any time          │
└─────────────────────────────────────────┘
```

**APR model:** Base APR is 8% (800 bps). Each 1% of utilization adds ~1.2 bps of bonus, capped at 20% (2000 bps). Higher transfer volume → higher vault utilization → higher LP yield.

---

## On-Chain Programs (Solana Devnet)

| Program | Address | Purpose |
|---|---|---|
| remittance_pool | `GKWPTDkKS2jDrE3gkWWoTtbHnwaiqZwc8iM47QGsJ9mJ` | Receives USDC from senders, escrows funds, tracks transfer state |
| liquidity_vault | `13BEbXJJ2aLQ6yMQA9QdtwguL2rDKdzsVBZNEbATwBhN` | LP deposits, advances to MTO, fee collection, rewards |

**SWI Token (test USDC):** `2Mfg6KX5hthtYnX8vAyqXreJtrYbxot5pbEzcyMpZGZx`

**On-chain transfer states:** `INITIATED → ADVANCED → DISBURSED → SETTLED`

---

## Monorepo Structure

```
swiflo/
├── apps/
│   ├── api/                        # Fastify backend
│   │   ├── src/
│   │   │   ├── routes/
│   │   │   │   ├── rate.ts         # GET /api/rates, /api/rate/current
│   │   │   │   ├── transfers.ts    # POST /api/transfers/estimate, /initiate
│   │   │   │   ├── faucet.ts       # POST /api/faucet (devnet SWI + SOL airdrop)
│   │   │   │   ├── webhook.ts      # POST /api/webhooks/mto (MTO callback)
│   │   │   │   ├── vault.ts        # GET /api/vault/state, /apr-history
│   │   │   │   └── lp.ts           # LP deposit/withdraw/status
│   │   │   ├── services/
│   │   │   │   ├── rateService.ts  # Pyth + ExchangeRate-API + fallbacks
│   │   │   │   ├── settler.ts      # Cron: settle DISBURSED transfers every 10s
│   │   │   │   └── vault.ts        # On-chain vault instructions (advanceToMto etc.)
│   │   │   ├── lib/
│   │   │   │   ├── prisma.ts       # Prisma client singleton
│   │   │   │   └── swifloChainLog.ts # Structured on-chain tx logging
│   │   │   └── index.ts            # Fastify app entry
│   │   ├── prisma/
│   │   │   └── schema.prisma       # DB schema
│   │   └── .env
│   │
│   ├── web/                        # Next.js 15 frontend
│   │   ├── app/
│   │   │   ├── page.tsx            # Landing page with savings calculator
│   │   │   ├── send/page.tsx       # Send flow (amount + recipient)
│   │   │   ├── fund/page.tsx       # LP funding / Get USDC flow
│   │   │   ├── lp/page.tsx         # LP dashboard
│   │   │   └── processing/[id]/    # Live transfer progress page
│   │   ├── components/
│   │   │   ├── CurrencyConverter.tsx
│   │   │   ├── Navbar.tsx
│   │   │   └── PrivyProvider.tsx
│   │   └── Dockerfile
│   │
│   └── mto-mock/                   # Mock MTO service (dev/demo)
│       └── src/
│           ├── index.ts            # POST /api/mto/disburse
│           ├── esewa-mock.ts       # Simulates eSewa credit
│           └── solana.ts           # confirmDisbursement on-chain
│
├── packages/
│   └── shared/                     # Shared TypeScript types
│       └── src/index.ts
│
├── programs/                       # Anchor (Rust) Solana programs
│   ├── remittance_pool/
│   └── liquidity_vault/
│
├── docker-compose.yml              # One-command startup
├── pnpm-workspace.yaml
└── turbo.json
```

---

## Database Schema

```
transfers
  id              String  PK (cuid)
  transferId      String  UNIQUE  (on-chain transfer ID)
  txSignature     String  UNIQUE  (Solana initiateTransfer signature)
  senderWallet    String
  recipientPhone  String
  amountUsdc      BigInt  (in token base units, 6 decimals)
  lockedRate      Float   (NPR/USD rate locked at transfer time)
  feeBps          Int     (default 40 = 0.40%)
  status          Enum: PENDING | DISBURSED | SETTLED | FAILED
  createdAt       DateTime
  disbursedAt     DateTime?
  settledAt       DateTime?

rateSnapshot
  id              String  PK
  usdcToNprRate   Float
  source          String  (pyth+exchangerate-api | exchangerate-api | coingecko | cached)
  recordedAt      DateTime

lpPosition
  id              String  PK
  walletAddress   String  UNIQUE
  depositedUsdc   BigInt
  shares          BigInt
  depositedAt     DateTime

vaultState
  id              String  PK ("singleton")
  totalLiquidity  BigInt  (LP_MINT total supply)
  activeAdvances  BigInt  (USDC currently advanced to MTOs)
  utilizationBps  Int
  currentAprBps   Int
  totalYieldPaid  BigInt
  updatedAt       DateTime
```

---

## Tech Stack

| Layer | Technology |
|---|---|
| Blockchain | Solana (Devnet → Mainnet) |
| Smart contracts | Anchor framework (Rust) |
| Wallets | Privy (embedded email/social → Solana keypair) |
| Token | SPL Token (SWI / test USDC, 6 decimals) |
| Oracle | Pyth Network (USDC/USD) + ExchangeRate-API (USD/NPR) |
| Backend | Fastify + TypeScript |
| ORM | Prisma |
| Database | Supabase (PostgreSQL) |
| Frontend | Next.js 15 (App Router) + Tailwind CSS |
| Monorepo | pnpm workspaces + Turborepo |
| Infra | Docker Compose |
| Events | Helius webhooks (on-chain indexing) |
| MTO payout | eSewa API (mock in dev — `apps/mto-mock`) |

---

## Running Locally

### Option A: Docker (recommended — one command)

```bash
# Clone the repo
git clone https://github.com/SrijitGyawali/SWIFLO.git
cd SWIFLO

# Copy and fill in your env vars
cp .env.example .env
# Edit .env with your keys

# Start everything (API + Web + MTO mock)
docker compose up --build

# API:      http://localhost:3001
# Web:      http://localhost:3000
# MTO mock: http://localhost:3002
```

### Option B: Manual

**Prerequisites:** Node 20+, pnpm 9+, PostgreSQL (or Supabase)

```bash
# Install dependencies
pnpm install

# Generate Prisma client
pnpm --filter @swiflo/api db:generate

# Push schema to DB
pnpm --filter @swiflo/api db:push

# Start API (terminal 1)
pnpm --filter @swiflo/api dev

# Start web (terminal 2)
pnpm --filter @swiflo/web dev

# Start MTO mock (terminal 3)
pnpm --filter @swiflo/mto-mock dev
```

---

## Environment Variables

| Variable | Where | Description |
|---|---|---|
| `SOLANA_RPC_URL` | API | RPC endpoint (devnet: `https://api.devnet.solana.com`) |
| `HELIUS_API_KEY` | API | Helius API key for webhooks |
| `DATABASE_URL` | API | PostgreSQL connection string |
| `REMITTANCE_POOL_PROGRAM_ID` | API | Deployed remittance_pool program address |
| `LIQUIDITY_VAULT_PROGRAM_ID` | API | Deployed liquidity_vault program address |
| `VAULT_SWI` | API | Vault's USDC token account address |
| `LP_MINT` | API | LP share token mint address |
| `SETTLER_KEYPAIR` | API | JSON byte array of settler wallet keypair |
| `FAUCET_SECRET_KEY` | API | JSON byte array of faucet wallet keypair |
| `MTO_MOCK_URL` | API | URL for MTO mock service (dev, default: http://localhost:3002) |
| `PRIVY_APP_ID` | API + Web | Privy application ID |
| `PRIVY_APP_SECRET` | API | Privy app secret (server-side only) |
| `NEXT_PUBLIC_API_URL` | Web | Backend API URL |
| `NEXT_PUBLIC_SOLANA_NETWORK` | Web | `devnet` or `mainnet-beta` |
| `NEXT_PUBLIC_PRIVY_APP_ID` | Web | Privy app ID (public) |
| `NEXT_PUBLIC_USDC_MINT` | Web | SWI token mint (shown in UI) |
| `MTO_KEYPAIR` | MTO mock | JSON byte array of MTO authority keypair |
| `API_URL` | MTO mock | Backend URL for disbursement callbacks |

---

## Demo: Send $100 to Nepal

1. Open `http://localhost:3000`
2. Click **Send money home**
3. Enter `100` USDC
4. Live rate appears: `1 USDC = Rs 150.99 · via pyth+exchangerate-api`
5. Enter recipient eSewa number: `9800000001`
6. Comparison shows: **Family gets Rs 213 more with Swiflo vs Western Union**
7. Click **Confirm & Send**
8. Privy signs the Solana `initiateTransfer` transaction — USDC locked in escrow
9. Backend advances liquidity token value from vault to MTO (`advanceToMto`)
10. MTO mock pays eSewa and calls `confirmDisbursement` on-chain
11. Settler cron (runs every 10s) picks up the DISBURSED transfer, calls `settleTransfer` + `replenishVault` + `collectFees`
12. Transfer status: **SETTLED** — family has NPR in eSewa wallet

---

## Getting Devnet SWI Tokens (Faucet)

```bash
# Request test tokens + SOL airdrop
curl -X POST http://localhost:3001/api/faucet \
  -H 'Content-Type: application/json' \
  -d '{"walletAddress": "YourSolanaWalletAddressHere", "amount": 100}'

# Response:
# {
#   "success": true,
#   "swiReceived": 100,
#   "solReceived": 1,
#   "txSignature": "5xK3...",
#   "explorerUrl": "https://explorer.solana.com/tx/5xK3...?cluster=devnet"
# }
```

The faucet transfers 100 SWI (test USDC) and 1 SOL (for transaction fees) from the faucet wallet. 5-second cooldown per wallet.

---

## Settlement Service

The settler runs every 10 seconds and processes transfers in `DISBURSED` state (after a 30-second delay in demo mode, 48 hours in production):

```
DISBURSED transfer found
  │
  ├── check on-chain status
  │     ├── already SETTLED → update DB, done
  │     └── DISBURSED → proceed
  │
  ├── settleTransfer (escrow → vault)
  ├── replenishVault (decrement active_advances)
  ├── collectFees (fee → treasury)
  └── update DB → SETTLED
```

The settler uses a tick-level mutex (`tickRunning`) and per-transfer in-flight tracking (`inFlight` Set) to prevent concurrent execution and double-settling.

---

## Helius Webhook Setup

Helius webhooks notify the API whenever a transaction hits the remittance_pool program:

```bash
# Register webhook (run once)
curl -X POST "https://api.helius.xyz/v0/webhooks?api-key=YOUR_KEY" \
  -H 'Content-Type: application/json' \
  -d '{
    "webhookURL": "https://your-api.com/api/webhook",
    "transactionTypes": ["TRANSFER"],
    "accountAddresses": ["6M9yzRSkn5c94dAvE8v9YJMGyoqHQKEurDTrM8AerQ56"],
    "webhookType": "enhanced",
    "authHeader": "your-webhook-secret"
  }'
```

---

## On-Chain Program Deployment (for contributors)

The Anchor programs are pre-deployed to devnet. To redeploy:

```bash
# Requires: Rust, Solana CLI, Anchor CLI, funded devnet wallet

cd programs
anchor build
anchor deploy --provider.cluster devnet

# Update program IDs in .env after redeployment
```

---

## Feature Comparison

| Feature | Western Union | Hawala | Swiflo |
|---|---|---|---|
| Fee | ~1.8% exchange rate markup | 3–5% opaque | **0.3% Swiflo platform (+ rail fees when enabled)** |
| Speed | 1–3 days | Same day | **~30 seconds** |
| Tracking | Email only | None | **On-chain (Solana)** |
| Minimum | $1 | Varies | **$1** |
| Payout method | Bank/cash | Cash | **eSewa (mobile)** |
| Exchange rate | 148.2560 NPR/USD (vs 150.99 mid-market) | Opaque | **Pyth oracle (live mid-market)** |
| Transparency | None | None | **Full on-chain audit trail** |

---

## What We're Building Toward

- [ ] Mainnet deployment
- [ ] Real eSewa API integration (live MTO)
- [ ] Mobile app (Expo)
- [ ] Multi-corridor: Gulf → Bangladesh, Gulf → India
- [ ] LP dashboard with real-time APR chart
- [ ] Privy SMS login for recipients without smartphones
- [ ] Real Bridge integration (settlement rail execution)
- [ ] Real Thunes integration (payout rail execution)

---

*Swiflo — keeping more money in the hands of Nepali families.*
